	...
	public void run(IAction action) {
		...
		if (editorInput instanceof IURIEditorInput) {
			...
			if (path != null) {
				...
				//IEditorDescriptor htmEd = editor.getSite().getWorkbenchWindow().getWorkbench().getEditorRegistry().getDefaultEditor(
				//		"C:/In/Projects/runtime-EclipseApplication/Tmp/src/tmp.htm");
				...
				try {
					...
					if(WINDOWS)
					{
						...
						pb.command().add("cmd");
						pb.command().add("/C");
						pb.command().add(FileLocator.getBundleFile(bundle).getAbsolutePath() + "\\runclonedigger.py " +
								"--links-for-eclipse " +
								"--output=\"" + htmFile + "\" " +
								"\"" + path + "\"" +
						" > nul 2>&1");
